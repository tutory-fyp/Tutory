...
import { Text as EText, Button as EButton } from 'react-native-elements';
...
class TutorSignupForm extends Component {
  ...
  _handleSignup() {
    ...
    if (this.state.cnic.length === 0) {
      Alert.alert('CNIC Empty', 'CNIC cannot be empty', [{ text: 'OK' }], {
        cancelable: true,
      });
      return;
    }
    if (this.state.cnic.length < 15) {
      Alert.alert('Invalid CNIC', 'CNIC Entered is Invalid', [{ text: 'OK' }], {
        cancelable: true,
      });
      return;
    }
    ...
    authentication
      ...
      .then(userCredential => {
        (async () => {
          try {
            await firestore()
              .doc(`tutors/${userCredential.user.uid}`)
              .set({
        ...
        let ref = storage().ref(
          `academicImages/${userCredential.user.uid}/${
    ...
    return (
      <View style={styles.signupCard}>
        ...
        <View style={styles.regAccWrapper}>
          <EText style={styles.regAccText}>Already Registered?</EText>
          <TouchableOpacity
            style={styles.regAccLoginBtn}
            onPress={() => this.props.navigation.navigate('Login')}>
            <EText style={styles.regAccLoginBtnText}>Log in</EText>
          </TouchableOpacity>
        </View>
      </View>
    );
